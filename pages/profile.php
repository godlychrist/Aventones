...
require_once ('../functions/editProfile.php' );
...
$correo = $user['mail'] ?? '';
$telefono = $user['phoneNum'] ?? '';
$imagenPerfil = $user['image'] ?? '/images/avatar_placeholder.png';
...
<body>
  <main class="container d-flex justify-content-center align-items-center min-vh-100 py-5">
    <div class="login-wrapper shadow border border-primary rounded p-4 p-md-5 text-center d-flex flex-column align-items-center gap-3" style="max-width: 720px; width:100%;">
      ...
      <form action="/functions/editProfile.php" method="post" enctype="multipart/form-data" class="formulario-login text-start w-100 mt-3" style="max-width: 600px;">

       
        <div class="d-flex align-items-center gap-3 mb-3">
          <div class="rounded-circle overflow-hidden" style="width: 80px; height: 80px; border:1px solid #0d6efd;">
          
            <img src="<?=htmlspecialchars($imagenPerfil)?>" alt="Foto de perfil" style="width:100%; height:100%; object-fit:cover;">
          </div>
          <div>
        ...
        <div class="row g-3">
          <div class="col-12 col-md-6">
            ...
            <input type="text" id="name" name="name" class="form-control" placeholder="Juan" value="<?= htmlspecialchars($nombre)?>" required>
          </div>
          <div class="col-12 col-md-6">
            <label for="lastname" class="form-label fw-bold text-dark">Apellidos</label>
            <input type="text" id="lastname" name="lastname" class="form-control" placeholder="Pérez Solano" value="<?= htmlspecialchars($apellidos)?>" required>
        ...
        <div class="row g-3 mt-1">
          <div class="col-12 col-md-6">
            ...
            <input type="text" id="cedula" name="cedula" class="form-control" value="<?= htmlspecialchars($cedula)?>" readonly>
          </div>
          <div class="col-12 col-md-6">
            <label for="birthDate" class="form-label fw-bold text-dark">Fecha de nacimiento</label>
            <input type="date" id="birthDate" name="birthDate" class="form-control" value="<?= htmlspecialchars($fechaNacimiento)?>">
          </div>
        </div>
        ...
        <div class="row g-3 mt-1">
          <div class="col-12 col-md-6">
            ...
            <input type="email" id="mail" name="mail" class="form-control" placeholder="tu@correo.com" value="<?= htmlspecialchars($correo)?>" required>
            <!-- value="<?php // echo htmlspecialchars($user['mail'] ?? ''); ?>" -->
          </div>
          <div class="col-12 col-md-6">
            <label for="phoneNum" class="form-label fw-bold text-dark">Teléfono</label>
            <input type="tel" id="phoneNum" name="phoneNum" class="form-control" placeholder="8888-8888" value="<?= htmlspecialchars($telefono)?>" required>
          </div>
        </div>
